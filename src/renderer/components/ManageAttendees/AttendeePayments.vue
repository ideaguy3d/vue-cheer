...
<main>
  <v-app>
    <v-layout row>
    <v-flex xs12>
      <v-btn light large style="margin: auto; margin-left: 40px" class="purple" v-on:click.native="back">Back to Attendees</v-btn>
    </v-flex>
    </v-layout>
    <v-layout row>
      <v-flex xs6>
    ...
    <v-layout row>
      <v-flex xs4>
        <p style="font-size: 25px">Name: {{ AttendeeInformation.Name }}</p>
      </v-flex>
      <v-flex xs4>
        <p style="font-size: 25px">Name: {{ AttendeeInformation.Country.CountryName }}</p>
      </v-flex>
    </v-layout>
    <v-layout row>
      <v-flex xs8>
...
import guid from "guid";
  export default {
    ...
    methods: {
      fetchData(){
        let component = this;
        request.post({json: true, url: "http://"+this.$store.state.serverIP+'/get/getAttendee', form: {token: this.$store.state.token, EventID: this.$store.state.eventData.EventID, AttendeeID: this.$store.state.manageAttendee.AttendeeID}}, function (err, httpResponse, body){
          if (httpResponse.statusCode == 200){
            component.AttendeeInformation = body;
            component.Paid = body.TotalPaid;
            component.Balance = body.RemainingBalance;
          }
      });
        request.post({json: true, url: "http://"+this.$store.state.serverIP+'/get/getAttendeePayments', form: {token: this.$store.state.token, EventID: this.$store.state.eventData.EventID, AttendeeID: this.$store.state.manageAttendee.AttendeeID}}, function (err, httpResponse, body){
          if (httpResponse.statusCode == 200){
    ...
    add (){
        let component = this;
        let TransactionData = {
          AttendeeID: this.$store.state.manageAttendee.AttendeeID,
          Type: this.Type.text,
          Amount: this.Amount,
        ...
        request.post({json: {token: this.$store.state.token, TransactionData: TransactionData, EventID: this.$store.state.eventData.EventID}, url: "http://"+this.$store.state.serverIP+'/post/AttendeePaymentTransaction', method: "POST"}, function (err, httpResponse, body){
            if (httpResponse.statusCode == 200){
    ...
    del(Trans_ID){
        let component = this;
        request.post({json: true, url: "http://"+this.$store.state.serverIP+'/delete/AttendeePaymentTransaction', form: {token: this.$store.state.token, EventID: this.$store.state.eventData.EventID, Trans_ID: Trans_ID}}, function (err, httpResponse, body){
          if (httpResponse.statusCode == 200){
            component.fetchData();
          }
      });
