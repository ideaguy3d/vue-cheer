    ...
    export default {
        ...
        methods: {
            ...
            save (){
                ...
                request.post({json: {token: this.$store.state.token, UserData: UserData}, url: "http://"+this.$store.state.serverIP+'/post/editUser', method: "POST"}, function (err, httpResponse, body){
                    if (httpResponse.statusCode == 200){
                        if (body != "DUPLICATE"){
