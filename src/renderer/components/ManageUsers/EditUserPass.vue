<template>
    <main>
        <h3>Edit Application User Password: </h3>
        <v-card class="grey lighten-4 elevation-0">
            <v-card-text>
                <v-container fluid>
                    ...
                    <v-flex xs8>
                        <v-text-field name="input-1" label="Password" id="testing" v-model="Password" type="password"></v-text-field>
                    </v-flex>
                    <!--<v-flex xs8>
    ...
    export default {
        ...
        methods: {
                    ...
                    let component = this;
                     request.post({json: true, url: "http://"+this.$store.state.serverIP+'/get/getPrivelages', form: {token: this.$store.state.token, EventID: this.$store.state.eventData.EventID}}, function (err, httpResponse, body){
                         if (httpResponse.statusCode == 200){
                             ...
                             //console.log(component.Privelages);
                             for (let i=0; i < component.Privelages.length; i++){
                                if (component.Privelages[i].LevelID == component.LevelID){
                                     component.Level = component.Privelages[i];
                                 }
                             }
                         }
                     })
            ...
            save (){
                ...
                request.post({json: {token: this.$store.state.token, UserData: UserData}, url: "http://"+this.$store.state.serverIP+'/post/setUserPass', method: "POST"}, function (err, httpResponse, body){
                    if (httpResponse.statusCode == 200){
                        if (body != "DUPLICATE"){
        ...
        created () {
            this.UserID = this.$store.state.manageUser.UserID;
            this.EmailAddress = this.$store.state.manageUser.EmailAddress;
            this.FirstName = this.$store.state.manageUser.FirstName;
            this.LastName = this.$store.state.manageUser.LastName;
            this.MiddleName = this.$store.state.manageUser.MiddleName;
            this.PhoneNumber = this.$store.state.manageUser.PhoneNumber;
            this.LevelID = this.$store.state.manageUser.LevelID;
            //console.log(this.$store.state.manageUser);
            this.fetchData();
        }
    }
</script>
